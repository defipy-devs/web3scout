...
class RemoveLiquidityEvent(Event):
    ...
    def filter(self, contract, addr = None, fromBlock = None, toBlock = None, argument_filters = None):
        if argument_filters:
            return contract.events.RemoveLiquidity.get_logs(fromBlock = fromBlock, toBlock = toBlock, argument_filters = argument_filters)
        return contract.events.RemoveLiquidity.get_logs(fromBlock = fromBlock, toBlock = toBlock)
